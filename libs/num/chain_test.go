...
func TestUintChain(t *testing.T) {
	...
	expect := Sum(start, vals[3], vals[5]) // some stuff
	...
	// write as one-liner
	got := UintChain(Sum(start, vals[3], vals[5])).Add(vals[9], vals[1]).Sub(vals[7]).Mul(vals[4]).Div(vals[2]).Get()
...
func TestDecChain(t *testing.T) {
	...
	chain := DecChain(start).Add(vals[3], vals[5], vals[9].Add(vals[1])).Sub(vals[7]).Mul(vals[4]).Div(vals[2])
	...
	t.Run("compare Uint rounding", func(t *testing.T) {
		...
		uf, _ := chain.GetUint(DecFloor)
		require.Equal(t, floor.String(), uf.String())
		ur, _ := chain.GetUint(DecRound)
		require.Equal(t, round.String(), ur.String())
		uc, _ := chain.GetUint(DecCeil)
...
func TestDecChainFromUint(t *testing.T) {
	...
	// we could just assign got at the end, but we want to check for rounding
	chain := UintDecChain(uStart).Add(vals[3], vals[5], vals[9].Add(vals[1])).Sub(vals[7]).Mul(vals[4]).Div(vals[2])
