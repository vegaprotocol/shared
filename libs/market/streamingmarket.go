...
func NewStream(log *logging.Logger, name string, node dataNode, pauseCh chan types.PauseSignal) *market {
	return &market{
		name:      name,
		node:      node,
...
func (m *market) Store() Store {
	return m.store
}

func (m *market) Subscribe(ctx context.Context, pubKey, marketID string) error {
	m.marketID = marketID
	m.pubKey = pubKey
...
func (m *market) FindMarket(ctx context.Context, quoteF, baseF string) (*vega.Market, error) {
	marketsResponse, err := m.node.Markets(ctx, &dataapipb.ListMarketsRequest{})
	if err != nil {
	...
	return nil, fmt.Errorf("failed to find futures markets: base/ticker=%s, quote=%s", baseF, quoteF)
}
...
func (m *market) subscribeToMarketEvents() {
	...
	proc := func(rsp *coreapipb.ObserveEventBusResponse) (bool, error) {
		for _, event := range rsp.Events {
			...
			m.store.MarketDataSet(cache.SetMarketData(md))
		}
		return false, nil
	}
...
func (m *market) subscribePositions() {
	...
	proc := func(ev *coreapipb.ObserveEventBusResponse) (bool, error) {
		...
		m.store.MarketDataSet(cache.SetOpenVolume(openVolume))
		return false, nil
	}
